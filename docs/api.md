...
| `POST /api/scan/retry/[chcId]` | Re-run the VLM against the JPEG already in Storage (`maxDuration = 60`) | `{ code, record, log? }` |
...
- `retry` requires the record to already have a `jpeg_url` (derivation is a local CLI step; serverless does not derive).
