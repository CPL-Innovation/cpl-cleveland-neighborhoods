...
function ViewBtn({ icon, label, active }) {
  const t = STAFF_TOKENS;
  const glyph = {
...
function FilterChip({ label, value, highlight, add }) {
  const t = STAFF_TOKENS;
  if (add) {
    return (
      <button style={{
...
function PhotosBulkBar() {
  const t = STAFF_TOKENS;
  const nav = (typeof useNav === 'function') ? useNav() : { selection: new Set(), clearSelection: () => {}, toast: () => {} };
...
function BulkAction({ children, onClick }) {
  const t = STAFF_TOKENS;
  return (
...
function PhotosSavedViews() {
  const t = STAFF_TOKENS;
  const nav = (typeof useNav === 'function') ? useNav() : { savedView: 'Tremont · missing geo', navigate: () => {} };
...
function SavedView({ name, count, active }) {
  const t = STAFF_TOKENS;
...
function SheetRow({ row, i, cols, t }) {
  const r = row;
  const nav = (typeof useNav === 'function') ? useNav() : { navigate: () => {}, toggleSelect: () => {} };
  return (
    <div
...
function CellContent({ col, r, t }) {
  switch (col) {
    ...
    case 'thumb': return r.thumbUrl ? (
      ...
      <div style={{
        width: 48, height: 36,
        background: `repeating-linear-gradient(${30 + r.thumb * 15}deg, #C8B68F 0 4px, #B8A37A 4px 8px)`,
        borderRadius: 3,
      }}/>
    );
...
Object.assign(window, { StaffPhotosList });
