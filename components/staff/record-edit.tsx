...
export function StaffRecordEdit() {
  const t = STAFF_TOKENS;
  const nav = useNav();
  const records = nav?.records || [];
  const idx = nav ? Math.max(0, records.findIndex(r => r.id === nav.recordId)) : 0;
  const cur: StaffRecord = records[idx] || { id: 'cpl_011_4738', title: 'Euclid Ave looking east', year: 'c.1915' };
...
function RecordSubBar({ cur, idx, total }: { cur: StaffRecord; idx: number; total: number }) {
  const t = STAFF_TOKENS;
  const nav = useNav();
  const records = nav.records || [];
  return (
...
function RecordPhotoPane({ cur }: { cur: StaffRecord }) {
  const t = STAFF_TOKENS;
...
function Annotation({ x, y, n, note, below, collapsed }: { x: string; y: string; n: string; note?: string; below?: boolean; collapsed?: boolean }) {
  const t = STAFF_TOKENS;
  return (
    <div style={{ position: 'absolute', left: x, top: y, transform: 'translate(-50%,-50%)' } as React.CSSProperties}>
      <div style={{
...
function IconBtn({ glyph, active }: { glyph: string; active?: boolean }) {
  const inner = ({
  ...
  } as Record<string, React.ReactNode>)[glyph];
  return (
    <button style={{
...
function RecordFieldsPane({ cur }: { cur: StaffRecord }) {
  const t = STAFF_TOKENS;
  ...
  return (
    <div key={cur?.id || 'empty'} style={{
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        } as React.CSSProperties}>ContentDM · locked</div>
        <div style={{
...
function SmallPin({ x, y, active, muted }: { x: string; y: string; active?: boolean; muted?: boolean }) {
  const t = STAFF_TOKENS;
...
function ConfidenceSegmented({ value }: { value: string }) {
  const t = STAFF_TOKENS;
  const levels = [
...
function StatusSegmented({ value, kind = 'public' }: { value: string; kind?: string }) {
  const t = STAFF_TOKENS;
...
function RecordContextPane({ cur }: { cur: StaffRecord }) {
  const t = STAFF_TOKENS;
  const nav = useNav();
  const records = nav.records || [];
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            letterSpacing: 1.2, textTransform: 'uppercase',
            color: t.inkMuted,
          } as React.CSSProperties}>ContentDM · source</div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        } as React.CSSProperties}>You might enrich next</div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          } as React.CSSProperties}>AI assist</span>
...
function SrcRow({ label, value, mono, last }: { label: string; value: string; mono?: boolean; last?: boolean }) {
  const t = STAFF_TOKENS;
  return (
    <div style={{
      display: 'flex', gap: 8,
...
function SuggestNext({ id, caption, reason }: { id: string; caption: string; reason: string }) {
  const t = STAFF_TOKENS;
  const nav = useNav();
  return (
    <div
...
function Comment({ who, when, body, self }: { who: string; when: string; body: string; self?: boolean }) {
  const t = STAFF_TOKENS;
  return (
    <div style={{
...
function AiSugg({ label, status, body }: { label: string; status: string; body: string }) {
  const t = STAFF_TOKENS;
  const nav = useNav();
