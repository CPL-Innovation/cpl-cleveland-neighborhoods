// Staff app chrome: top bar + left sidebar nav.
// Reused across all staff screens. Compact, dense, utilitarian.
...
function StaffShell({ activeSection, screenTitle, screenMeta, children, user = 'Brian Meggitt' }) {
  return (
    <div style={{
...
function StaffSidebar({ activeSection, user }) {
    ...
    <aside style={{
      ...
      <div style={{
        ...
        <div style={{ lineHeight: 1.1 }}>
          <div style={{
            fontFamily: t.serif, fontSize: 15.5, fontWeight: 500,
            letterSpacing: -0.2, color: '#F6F2EB',
          }}>Cleveland Neighborhoods</div>
          <div style={{
            fontFamily: t.mono, fontSize: 9.5,
            letterSpacing: 1.2, textTransform: 'uppercase',
            color: t.inkFaint, marginTop: 2,
          }}>Enrichment · Staff</div>
        </div>
      </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          background: t.sage, boxShadow: `0 0 0 3px ${t.sage}22`,
        }}/>
      </div>
      <div style={{
        color: '#F6F2EB', fontSize: 12.5, lineHeight: 1.4,
      }}>
        <span style={{ fontWeight: 600 }}>3 records</span> changed upstream
      </div>
      <div style={{
        color: t.inkFaint, fontSize: 11, marginTop: 2, fontFamily: t.mono,
      }}>last pull · 04:12 today</div>
    </div>
  );
}

function StaffTopBar({ title, meta }) {
  const t = STAFF_TOKENS;
  const nav = (typeof useNav === 'function') ? useNav() : { navigate: () => {}, toast: () => {} };
  return (
    <div style={{
      ...
      <div style={{
        ...
        letterSpacing: -0.2, color: t.ink, lineHeight: 1,
      }}>{title}</div>
      {meta && (
        <div style={{
      ...
      {/* Search */}
      <button onClick={() => nav.toast('Command palette — not wired in this MVP', 'info')} style={{
        height: 34, padding: '0 12px',
        display: 'flex', alignItems: 'center', gap: 8,
        background: t.bgPanel,
        border: `1px solid ${t.border}`,
        borderRadius: 17,
        color: t.ink, fontSize: 12.5,
      ...
      <div style={{
        ...
        fontFamily: t.mono, fontSize: 10.5,
        color: t.inkMuted, letterSpacing: 0.3,
      }}>
      ...
      {/* New */}
      <button onClick={() => nav.toast('Stub — new record creation flow not built', 'info')} style={{
        ...
        <svg width="11" height="11" viewBox="0 0 11 11">
          <path d="M5.5 1v9M1 5.5h9" stroke="#F6F2EB" strokeWidth="1.5" strokeLinecap="round"/>
        </svg>
        New
      </button>
    </div>
...
function SearchIcon14({ color = '#3D3833' }) {
  return (
