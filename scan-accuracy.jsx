    ...
    <div style={{ width: '100%', height: '100%', overflowY: 'auto', background: t.bg }}>
      <div style={{ padding: '32px 40px 48px', maxWidth: 860, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <a href={scanApi.csvUrl} download style={{ ...pillBtn(t, false), textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>Export CSV ↓</a>
        </div>
        ...
        <div style={{ fontFamily: t.mono, fontSize: 10, letterSpacing: 1.4, textTransform: 'uppercase', color: t.inkMuted, marginBottom: 10 }}>
        ...
        {acc.description.notes.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontFamily: t.mono, fontSize: 10, letterSpacing: 1.4, textTransform: 'uppercase', color: t.inkMuted, marginBottom: 10 }}>Review notes corpus ({acc.description.notes.length})</div>
        ...
        {acc.misses.length ? acc.misses.map((m, i) => (
          <div key={i} onClick={() => nav.navigate('scanReview', { id: m.chc_id })} style={{
            ...
            <span style={{ fontFamily: t.mono, fontSize: 10, color: t.inkMuted, textTransform: 'uppercase' }}>{m.field}</span>
...
Object.assign(window, { ScanAccuracy });
